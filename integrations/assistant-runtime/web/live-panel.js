/** Live conversation lifecycle scoped to the current Integration panel. */
export function mountLive(root) {
const start=root.querySelector('#start'),end=root.querySelector('#end'),status=root.querySelector('#status'),audio=root.querySelector('#audio'),caption=root.querySelector('#transcript');
let authenticated=true;
function showLogin(){ authenticated=false;start.disabled=true;status.textContent='Your sign-in expired. Reload Carvis to sign in again.'; }
async function checkAccess(){
  const response=await fetch('/integrations/assistant-engine/api/live/status',{cache:'no-store',signal:AbortSignal.timeout(10000)});
  if(response.status===401){showLogin();return;}
  if(!response.ok)throw Error('Voice is unavailable. Check this integration’s settings.');
  const result=await response.json();start.disabled=!result.available;
  status.textContent=result.available?'Ready. Live voice is billed while connected.':'Add a live voice model and API key in Settings first.';
}
...
const confirmation=root.querySelector('#confirmation'),accept=root.querySelector('#accept'),decline=root.querySelector('#decline');
function showConfirmation(value,delegationId){
  ...
  if(!value)return;
  root.querySelector('#confirmationText').textContent=value.prompt;
...
return ()=>{window.removeEventListener('pagehide',onHide);send({type:'session.close'});cleanup();};
}
