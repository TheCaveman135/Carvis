...
export async function mount(ui) {
  ...
  async function memories() {
    ...
    const query=el('input',{type:'text',placeholder:'What would Carvis recall about this?'}),recall=el('div');
    content.append(card('Explore recalled context','See which memories, history, and evidence Carvis selects for a request.',submitForm([field('Request or topic',query)],'Preview recall',async()=>{const result=await get('/api/memories/context?q='+encodeURIComponent(query.value));const c=result.context;recall.replaceChildren(el('pre',{class:'recall-preview'},[c.rules,c.preferences,c.facts].filter(Boolean).join('\n\n') || 'No relevant memories yet.'),details('Why these memories were selected',c.trace));}),recall));
    ...
    content.append(card('Continuity Memory','Dated memories, connected context, and tentative routines. Old facts remain as history when new information replaces them.',details('Memory health',state.state)),card('Add a memory','Tell Carvis about a preference, fact, or rule.',submitForm([field('Memory',text),field('Type',kind)],'Remember this',async()=>{await post('/api/memories',{text:text.value,kind:kind.value});await reload();})),
      card('What Carvis remembers','Review, edit, or remove stored memories.',records(list(state.memories),m=>{
        ...
        return el('article',{class:'control-card'},el('p',{class:'small muted'},`${words(m.kind)} · ${words(m.source || 'Carvis')} · ${words(m.dmr?.state || 'current')}`),m.dmr?details('Memory details',{...m.dmr,createdAt:new Date(m.ts).toISOString(),updatedAt:new Date(m.updated_at).toISOString(),useCount:m.use_count}):null,editor,field('Keep pinned',pinned),el('div',{class:'action-row'},action('Save',async()=>{await post('/api/memories/update',{id:m.id,text:editor.value,kind:m.kind,pinned:pinned.checked});await reload();}),action('Forget',async()=>{if(window.confirm('Remove this memory?')){await post('/api/memories/delete',{id:m.id});await reload();}})));
      })),card('Learned routines','Patterns are observations. They do not give Carvis permission to act.',records(list(state.patterns),p=>row(p.summary || p.text || p.description || p.id,p.status || '',details('Pattern evidence',p),action('Dismiss',async()=>{await post('/api/patterns/dismiss',{id:p.id});await reload();})))));
  }
