...
export function entityStateDisplay({ state, unit } = {}, now = new Date()) {
  ...
  const label =
    localDay === now.toDateString()
      ? "Today"
      : localDay === yesterday.toDateString()
        ? "Yesterday"
        : localDay === tomorrow.toDateString()
          ? "Tomorrow"
  ...
  return {
    text: dateOnly
      ? label
      : `${label} at ${date.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })}`,
    title: dateOnly
