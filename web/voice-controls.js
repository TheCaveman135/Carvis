...
export function mountVoiceControls({el,button,api}) {
 ...
 function show(value){
  ...
  status.textContent=value.muted?'Microphone muted':!value.voiceEnabled?'Voice requests are switched off. Open Voice settings.':!value.transcriptionEnabled?'Transcription is switched off. Open Voice settings.':!value.transcriptionReady?'Speech recognition needs configuration. Open Voice settings.':!value.inputDevice?'Choose a microphone to start.':value.inputDevice==='even-glasses'?'Using Even glasses. The paired companion supplies audio.':value.error?value.error:value.processing?'Processing your request…':value.speaking?'Carvis is speaking…':value.hearingSpeech?'Hearing you…':value.bytesReceived>320000&&value.peakRms<1?`No microphone signal · ${name}. Check the device’s mute switch or choose another input.`:value.listening?`Listening · ${name}`:value.reconnecting?`Reconnecting · ${name}`:`Waiting for audio · ${name}`;
